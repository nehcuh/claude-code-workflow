...
module Vibe
  ...
  module RtkInstaller
    ...
    # @param config [Hash] Integration configuration (optional, for backward
    #                      compatibility)
    def install_rtk_interactive(config = nil)
      ...
      choice = ask_choice('   Choose [1-3]', %w[1 2 3])
    ...
    def install_rtk_via_homebrew_interactive
      ...
      if system('which', 'brew', out: File::NULL, err: File::NULL) ||
         system('where', 'brew', out: File::NULL, err: File::NULL)
        ...
        puts '   ✗ Homebrew not found. Please install Homebrew first or ' \
    ...
    def install_rtk_via_cargo_interactive
      ...
      if system('which', 'cargo', out: File::NULL, err: File::NULL) ||
         system('where', 'cargo', out: File::NULL, err: File::NULL)
        puts '   Installing RTK via Cargo...'
        rtk_config = read_yaml_abs(File.join(@repo_root, 'core/integrations/rtk.yaml'))
        cargo_url = rtk_config.dig('installation_methods', 'cargo',
                                   'command')&.split&.last || 'https://github.com/rtk-ai/rtk'
        ...
        puts '   ✗ Cargo not found. Please install Rust toolchain first or ' \
             'choose another method.'
      end
    end
