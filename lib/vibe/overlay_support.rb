...
module Vibe
  ...
  module OverlaySupport
    ...
    def resolve_overlay(explicit_path:, search_roots:)
      ...
      unless doc.is_a?(Hash)
        raise ValidationError,
              "Overlay file must contain a YAML mapping: #{path}"
      end
      ...
      if schema_version &&
        ...
        raise ValidationError,
              'Overlay schema_version out of range ' \
              "(1-#{MAX_OVERLAY_SCHEMA_VERSION}): #{path}"
      end
    ...
    def discover_overlay(search_roots)
      roots = Array(search_roots).compact.map { |root| File.expand_path(root) }.uniq
      roots.each do |root|
    ...
    def overlay_target_patch(overlay, target)
      ...
      unless patch.is_a?(Hash)
        raise ValidationError,
              "Overlay targets.#{target} must be a mapping"
      end
    ...
    def normalize_overlay_profile_mapping_overrides(overrides)
      ...
      unless overrides.is_a?(Hash)
        raise ValidationError,
              'Overlay profile.mapping_overrides must be a mapping'
      end
    ...
    def normalize_overlay_profile_note_append(notes)
      ...
      unless notes.is_a?(Array)
        raise ValidationError,
              'Overlay profile.note_append must be a list'
      end
    ...
    def normalize_overlay_policy_appends(policies, overlay_ref:)
      ...
      unless policies.is_a?(Array)
        raise ValidationError,
              'Overlay policies.append must be a list'
      end

      policies.map do |policy|
        unless policy.is_a?(Hash)
          raise ValidationError,
                'Each overlay policy must be a mapping'
        end
        ...
        unless missing.empty?
          raise ValidationError,
                "Overlay policy is missing required keys: #{missing.join(', ')}"
        end
    ...
    def validate_overlay_targets(targets, path)
      ...
      unless targets.is_a?(Hash)
        raise ValidationError,
              "Overlay 'targets' must be a mapping: #{path}"
      end
      ...
      targets.each do |target_name, patch|
        unless patch.is_a?(Hash)
          raise ValidationError,
                "Overlay targets.#{target_name} must be a mapping: #{path}"
        end
      end
    end
