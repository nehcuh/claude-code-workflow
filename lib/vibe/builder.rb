  ...
  module Builder
    ...
    def resolve_output_root_for_use(target:, destination_root:, explicit_output:)
      ...
      if home_dir.nil? || home_dir.empty?
        raise Vibe::ConfigurationError,
              'Cannot determine home directory for external staging'
      end
    ...
    def resolve_profile(target, requested_profile)
      ...
      if requested_profile
        ...
        if profile.nil?
          raise Vibe::ConfigurationError,
                "Unknown profile: #{requested_profile}"
        end
    ...
    def default_profile_for_target(target)
      profiles = providers.fetch('profiles').select do |_name, profile|
        profile['target'] == target
      end
      if profiles.empty?
        raise Vibe::ConfigurationError,
              "No profile found for target #{target}"
      end
    ...
    def manifest_skills(target)
      included = skills_doc.fetch('skills').select do |skill|
        include_skill_in_manifest?(skill)
      end
      included.map do |skill|
        {
          'id' => skill['id'],
    ...
    def include_skill_in_manifest?(skill)
      ...
      when 'gstack_installed'
        detect_gstack != :not_installed
      else
        false
      end
    end
    ...
    def inspect_payload(target:, overlay:)
...

      {
        ...
        'current_repo_target' => read_json_if_exists(
          File.join(@repo_root, self.class::MARKER_FILENAME)
        ),
