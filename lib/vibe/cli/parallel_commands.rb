  ...
  module ParallelCommands
    ...
    def run_worktree_command(argv)
      ...
      when nil, "help", "--help", "-h" then puts worktree_usage
        ...
        raise Vibe::ValidationError, "Unknown worktree subcommand: #{subcommand}\n\n#{worktree_usage}"
      end
    end
    ...
    def run_worktree_cleanup(argv)
      manager = WorktreeManager.new
      removed = manager.cleanup
      puts "🧹 Removed #{removed} finished worktree(s)"
    end

    def run_worktree_status(argv)
      manager = WorktreeManager.new
      s = manager.status
    ...
    def run_cascade_command(argv)
      ...
      when nil, "help", "--help", "-h" then puts cascade_usage
        ...
        raise Vibe::ValidationError, "Unknown cascade subcommand: #{subcommand}\n\n#{cascade_usage}"
      end
    end
    ...
    def run_cascade_run(argv)
      ...
      puts "Tasks: #{executor.tasks.size}  |  max_parallel: #{max_parallel || 'unlimited'}"
      puts
      ...
      puts
      puts "Result: #{result[:passed]} passed, #{result[:failed]} failed, #{result[:skipped]} skipped"
    ...
    def run_cascade_plan(argv)
      ...
      executor.topological_order.each_with_index do |id, i|
        ...
        deps = task["depends_on"].empty? ? "" : " (after: #{task['depends_on'].join(', ')})"
        puts "  #{i + 1}. #{id}#{deps}"
        puts "     $ #{task['command']}"
      end
    end
