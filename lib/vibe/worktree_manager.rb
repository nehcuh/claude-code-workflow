  ...
  class WorktreeManager
    ...
    def list(filters = {})
      ...
      entries = Dir.glob(File.join(@worktrees_dir, '*', '.vibe-worktree.yaml'))
                   .map do |f|
        YAML.safe_load(File.read(f), permitted_classes: [Time, Symbol],
                                     aliases: true)
      end
                   .compact
    ...
    def get(id)
      ...
      YAML.safe_load(File.read(meta_path), permitted_classes: [Time, Symbol],
                                           aliases: true)
    end
    ...
    def remove(id, options = {})
      ...
      run_git!('branch', '-D', info['branch']) unless options[:keep_branch]
    ...
    def current_branch
      out, _status = Open3.capture2e('git', '-C', @repo_root, 'rev-parse',
                                     '--abbrev-ref', 'HEAD')
      out.strip
    end
