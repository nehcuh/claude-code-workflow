...
class TestGstackIntegration < Minitest::Test
  ...
  def test_registry_gstack_skills_have_valid_structure
    ...
    assert gstack_skills.length.positive?, 'Should have gstack skills in registry'
    ...
    gstack_skills.each do |skill|
      ...
      assert_equal 'external', skill['entrypoint'],
                   "Entrypoint should be external: #{skill['id']}"
      assert_equal 'gstack_installed', skill['conditional'],
                   "Should be conditional: #{skill['id']}"
    end
  end
...
class TestGstackDetection < Minitest::Test
  ...
  def test_detect_gstack_not_installed
    ...
    skip "gstack is installed at #{gstack_location} — skipping negative test" if gstack_location
  ...
  def test_gstack_detection_paths_defined
    assert Vibe::ExternalTools::GSTACK_DETECTION_PATHS.length.positive?
    assert Vibe::ExternalTools::GSTACK_MARKER_FILES.length.positive?
  end
