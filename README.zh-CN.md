...
- **主要变更**：
  - 将 CLI 模块化为 9 个 Ruby 库模块（`lib/vibe/*.rb`）
  - 添加完整的单元测试套件（`test/`）
...
- 自动保存进度——关闭窗口不会丢失任何工作
- **根据上下文自动建议外部技能**（例如，实现功能时建议 TDD 工作流，创建 PR 前建议代码审查工作流）
...
最简单的入门方式是使用 `quickstart` 命令，它会自动将工作流安装到你的 Claude Code 配置目录 (`~/.claude`)。
...
编辑 `~/.claude/CLAUDE.md`，填入你的信息：

- **User Info**：你的名字、项目目录、社交账号
- **Sub-project Memory Routes**：将你的项目映射到内存路径
- **SSOT Ownership Table**：定义每类信息的存储位置
- **On-demand Loading Index**：根据需要调整文档路径
...
Claude 将自动加载规则并开始遵循工作流。

---

### 高级：手动安装

如果你希望将工作流文件维护在特定位置或使用符号链接：

```bash
# 复制到 Claude Code 配置目录
cp -r claude-code-workflow/* ~/.claude/

# 或使用符号链接以保持与 git 仓库同步
...
**自动触发场景**：安装外部技能包后，系统会自动生成触发规则，告诉 LLM 何时使用这些技能。例如：
- "当实现新功能时" → 建议使用 `superpowers/tdd`
- "在创建 PR 之前" → 建议使用 `superpowers/review`
- "当重构代码时" → 建议使用 `superpowers/refactor`

详见 [docs/integrations.md](docs/integrations.md)，如需添加自己的推荐，请参阅 [docs/extending-recommendations.md](docs/extending-recommendations.md)。如需添加新的外部技能，请参阅 [docs/adding-external-skills.md](docs/adding-external-skills.md)。
